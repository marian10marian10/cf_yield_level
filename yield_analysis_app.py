...
def create_parcel_performance_map(df):
    """Vytvorenie mapy s výkonnosťou parciel"""
    ...
    for _, row in parcel_stats.iterrows():
        try:
            lat, lon = parse_geometry(row['geometry'])
...
def main():
    ...
    # Výber plodiny
    st.header("🌱 Analýza podľa plodiny")
    
    available_crops = sorted(df['crop'].unique())
    selected_crop = st.selectbox("Vyberte plodinu:", available_crops)
    ...
    if selected_crop:
        ...
        # Detailné dáta
        st.subheader("Detailné dáta")
        st.dataframe(
            crop_data[['name', 'year', 'yield_ha', 'yield_percentage', 'area']].sort_values('year', ascending=False),
    ...
    # Porovnanie plodín
    st.header("📈 Porovnanie plodín")
    
    # Priemerné výnosy podľa plodín
    crop_comparison = df.groupby('crop').agg({
        'yield_ha': 'mean',
        'yield_percentage': 'mean',
        'area': 'sum'
    }).round(2)
    ...
    with col1:
        fig = px.bar(
            crop_comparison,
            x=crop_comparison.index,
            y='yield_ha',
            title="Priemerné výnosy podľa plodín (t/ha)"
        )
        fig.update_layout(height=400)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        fig = px.bar(
            ...
            x=crop_comparison.index,
            y='yield_percentage',
            title="Priemerná výnosnosť podľa plodín (%)"
        )
        fig.update_layout(height=400)
        st.plotly_chart(fig, use_container_width=True)
